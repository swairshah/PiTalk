...
enum GoogleApiKeySource {
    case processEnvironment
    case userDefaults
    case dotEnv
}

struct GoogleApiKeyResolution {
    let key: String
    let source: GoogleApiKeySource
}

enum GoogleApiKeyBootstrapResult {
...
enum GoogleApiKeyManager {
    static let userDefaultsKey = "googleTtsApiKey"
    private static let envKeys = ["GOOGLE_TTS_API_KEY"]

    static func resolved() -> GoogleApiKeyResolution? {
        if let key = keyFromProcessEnvironment() {
            return GoogleApiKeyResolution(key: key, source: .processEnvironment)
        }

        if let key = keyFromUserDefaults() {
            return GoogleApiKeyResolution(key: key, source: .userDefaults)
        }

        if let key = keyFromDotEnv() {
            return GoogleApiKeyResolution(key: key, source: .dotEnv)
        }
    ...
    static func bootstrapPersistedKeyIfNeeded() -> GoogleApiKeyBootstrapResult {
    ...
    static func importFromDotEnv(overwriteExisting: Bool = false) -> GoogleApiKeyImportResult {
