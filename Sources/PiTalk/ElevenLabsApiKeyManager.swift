...
enum ElevenLabsApiKeySource {
    case processEnvironment
    case userDefaults
    case dotEnv
}

struct ElevenLabsApiKeyResolution {
    let key: String
    let source: ElevenLabsApiKeySource
}

enum ElevenLabsApiKeyBootstrapResult {
...
enum ElevenLabsApiKeyImportResult {
    case imported
...
enum ElevenLabsApiKeyManager {
    static let userDefaultsKey = "elevenLabsApiKey"
    private static let envKeys = ["ELEVEN_API_KEY", "ELEVENLABS_API_KEY"]

    static func resolved() -> ElevenLabsApiKeyResolution? {
        if let key = keyFromProcessEnvironment() {
            return ElevenLabsApiKeyResolution(key: key, source: .processEnvironment)
        }

        if let key = keyFromUserDefaults() {
            return ElevenLabsApiKeyResolution(key: key, source: .userDefaults)
        }

        if let key = keyFromDotEnv() {
            return ElevenLabsApiKeyResolution(key: key, source: .dotEnv)
        }
    ...
    static func bootstrapPersistedKeyIfNeeded() -> ElevenLabsApiKeyBootstrapResult {
    ...
    static func importFromDotEnv(overwriteExisting: Bool = false) -> ElevenLabsApiKeyImportResult {
